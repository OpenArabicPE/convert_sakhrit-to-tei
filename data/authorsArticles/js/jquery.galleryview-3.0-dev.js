	...
	*/		
	GalleryView = {
		...
		elems: [
			...
			'.gv_navWrap',	'.gv_navPrev'	,		'.gv_navNext',			'.gv_navPlay',
			'.gv_panelNavPrev',	'.gv_panelNavNext',	'.gv_overlay',			'.gv_showOverlay'
		],
		...
		setDimensions: function() {
			...
			if(this.filmstripOrientation === 'vertical' && (this.opts.frame_width + gv.extraWidth(dom.gv_frame)) < (widths.prev + widths.play + widths.next)) {
				...
				dom.gv_navWrap.css({
					width: widths.prev + widths.play + widths.next+4,
...
//			dom.gv_panelNavNext.css({ top: (gv.outerHeight(dom.gv_panel) - gv.outerHeight(dom.gv_panelNavNext)) / 2, left: 8 });
//			dom.gv_panelNavPrev.css({ top: (gv.outerHeight(dom.gv_panel) - gv.outerHeight(dom.gv_panelNavPrev)) / 2, right: 8 });
            dom.gv_panelNavNext.css({ bottom: 15, left: 8 });
			dom.gv_panelNavPrev.css({ bottom: 15, right: 8 });
...
//					dom.gv_filmstripWrap.css({ top: fsVert, left: fsHorz });
		...
		showInfoBar: function() {
			...
			dom.gv_infobar.stop().stopTime('hideInfoBar_' + self.id).html(' page '+(this.iterator+1) +' of  '+ this.numImages).show().css('opacity',1);
		...
		updateOverlay: function(i) {
			...
			if(this.overlayVisible) {
				this.hideOverlay(function(){
					dom.gv_overlay.html('<h4 dir=rtl>'+self.gvImages[i].attrs.title+'</h4><p dir=rtl>'+self.gvImages[i].attrs.description+'</p>');
					self.showOverlay();
				});
			} else {
				dom.gv_overlay.html('<h4 dir=rtl>'+self.gvImages[i].attrs.title+'</h4><p><h5 dir=rtl>'+self.gvImages[i].attrs.description+'</h5></p>');
					...
					dom.gv_filmstrip.animate({
						...
						complete: function(){
							if(to < self.numImages) {
								dom.gv_filmstrip.css('right',gv.getInt(dom.gv_filmstrip.css('left'))-(self.numImages*(gv.outerWidth(dom.gv_frame)+self.opts.frame_gap)));	
